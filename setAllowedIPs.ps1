...
try {
    # Recebe o ID do usuario e os IPs
    $idUsuario = Get-VeracodeUserID $emailUsuario
    $caminhoJSON = "$pastaTemplates\AllowedIPs.json"
    ...
    # Atualiza os IPs
    $urlAPI = "https://api.veracode.com/api/authn/v2/users/" + $idUsuario + "?partial=true&adminNoVerificationEmail=true"
